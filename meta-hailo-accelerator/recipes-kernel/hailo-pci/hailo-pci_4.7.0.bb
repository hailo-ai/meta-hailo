...
SRCREV = "1713379fd9e2df130d0ba5bf70a7e1f499bcdf27"
...
python() {
  # PCIe driver compilation requires soft FPU, the variable must be set to "soft" in case the platform is an arm architecture with hard FPU target.
  if d.getVar("TARGET_FPU") == 'hard' or d.getVar("TARGET_ARCH") == 'arm':
    return d.setVar("TARGET_CC_ARCH", "-march=armv7-a -mfpu=vfp -mfloat-abi=soft")
}
